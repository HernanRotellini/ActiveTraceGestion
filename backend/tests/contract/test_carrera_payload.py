"""Contract test: POST /api/admin/carreras payload matches backend schema."""
...
async def schema(db_engine: None) -> None:
    ...
    from app.models.estructura_academica import Carrera  # noqa: F401
...
async def context(db_session: AsyncSession, schema: None) -> dict[str, Any]:
    tenant = Tenant(name="Tenant test", code=f"carrera-{uuid4().hex[:8]}")
...
class TestCarreraContract:
    async def test_correct_payload_returns_201(
        ...
        assert data["nombre"] == "Test Carrera"
    ...
    async def test_unknown_field_returns_422(
        ...
        resp = await async_client.post(
            "/api/admin/carreras",
            json={"codigo": "TEST", "nombre": "Test", "descripcion": "extra"},
            headers=hdrs,
        )
