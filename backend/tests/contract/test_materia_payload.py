"""Contract test: POST /api/admin/materias payload matches backend schema."""
...
async def context(db_session: AsyncSession, schema: None) -> dict[str, Any]:
    tenant = Tenant(name="Tenant test", code=f"materia-{uuid4().hex[:8]}")
...
class TestMateriaContract:
    async def test_correct_payload_returns_201(
        ...
        resp = await async_client.post(
            "/api/admin/materias",
            json={"codigo": "TEST", "nombre": "Test Materia"},
            headers=hdrs,
        )
        ...
        assert data["nombre"] == "Test Materia"
    ...
    async def test_unknown_field_returns_422(
        ...
        resp = await async_client.post(
            "/api/admin/materias",
            json={"codigo": "TEST", "nombre": "Test", "carga_horaria": 4},
            headers=hdrs,
        )
