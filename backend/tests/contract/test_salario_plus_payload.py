"""Contract test: POST /api/liquidaciones/grilla/pluses payload matches backend schema."""
...
from app.models.permisos import LIQUIDACIONES_OPERAR_GRILLA
...
async def schema(db_engine: None) -> None:
    ...
    from app.models.liquidaciones import SalarioPlus  # noqa: F401
...
async def context(db_session: AsyncSession, schema: None) -> dict[str, Any]:
    tenant = Tenant(name="Tenant test", code=f"salplus-{uuid4().hex[:8]}")
    ...
    permiso = Permiso(
        tenant_id=tenant.id,
        codigo=LIQUIDACIONES_OPERAR_GRILLA,
        nombre="Operar grilla salarial",
        modulo="liquidaciones",
        accion="operar_grilla",
    )
...
class TestSalarioPlusContract:
    async def test_correct_payload_returns_201(
        ...
        resp = await async_client.post(
            ...
            json={"rol": "TUTOR", "grupo": "antiguedad", "descripcion": "Plus antig\u00fcedad", "monto": "10000.00", "desde": "2026-01-01"},
            headers=hdrs,
        )
