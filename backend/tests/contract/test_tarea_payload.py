"""Contract test: POST /api/tareas payload matches backend schema."""
...
async def context(db_session: AsyncSession, schema: None) -> dict[str, Any]:
    tenant = Tenant(name="Tenant test", code=f"tarea-{uuid4().hex[:8]}")
    ...
    # The actor (AuthUser) must have a corresponding Usuario with the same ID
    actor_usr = Usuario(
        id=user.id,
        tenant_id=tenant.id,
        nombre="Admin",
...
class TestTareaContract:
    async def test_correct_payload_returns_201(
        ...
        resp = await async_client.post(
            "/api/tareas",
            json={"titulo": "Test Tarea", "descripcion": "Test descripcion", "asignado_a": context["asignado_a_id"]},
            headers=hdrs,
        )
        ...
        assert data["asignado_a"] == context["asignado_a_id"]
    ...
    async def test_unknown_field_returns_422(
        ...
        resp = await async_client.post(
            ...
            json={
                ...
                "asignado_a": context["asignado_a_id"],
                "prioridad": "alta",
            },
            headers=hdrs,
        )
