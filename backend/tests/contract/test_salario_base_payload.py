"""Contract test: POST /api/liquidaciones/grilla/bases payload matches backend schema."""
...
async def schema(db_engine: None) -> None:
    ...
    from app.models.liquidaciones import SalarioBase  # noqa: F401
...
async def context(db_session: AsyncSession, schema: None) -> dict[str, Any]:
    tenant = Tenant(name="Tenant test", code=f"salbase-{uuid4().hex[:8]}")
...
class TestSalarioBaseContract:
    async def test_correct_payload_returns_201(
        ...
        resp = await async_client.post(
            ...
            json={"rol": "TUTOR", "monto": "50000.00", "desde": "2026-01-01"},
            headers=hdrs,
        )
    ...
    async def test_unknown_field_returns_422(
        ...
        resp = await async_client.post(
            "/api/liquidaciones/grilla/bases",
            json={"rol": "TUTOR", "monto": "50000.00", "desde": "2026-01-01", "importe": "50000.00"},
            headers=hdrs,
        )
